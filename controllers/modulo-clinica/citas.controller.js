...
class CitaMedicaController extends RestBuilder {
  constructor() {
    super()
    this.setTable('citas').setName('Cita medica').setPagination()
    .setTimeStamps();
  }
  ...
  renderHomeView = async (req, res) => {
    try {
      ...
      if (role == 1 || role == 2) {
        return this.renderView(res, 'modulo-clinica/citas/superadmin', {
          dataSession,
          dataSistema
        });
      }
      return this.renderView(res, 'modulo-clinica/citas', {
        dataSession,
        dataSistema
      });
