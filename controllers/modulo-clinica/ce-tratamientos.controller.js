...
class TratamientoController extends RestBuilder {
  constructor() {
    super()
    this.setTable('ce_tratamientos').setName('Tratamiento')
    .setTimeStamps();
  }
  ...
  renderHomeView = async (req, res) => {
    try {
      ...
      if (role == 1 || role == 2) {
        return this.renderView(res, 'modulo-clinica/tratamientos/superadmin', {
          dataSession,
          dataSistema
        });
      }

      return this.renderView(res, 'modulo-clinica/tratamientos', {
        dataSession,
        dataSistema
      });
