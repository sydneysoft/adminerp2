...
class FaqController extends RestBuilder {
  constructor() {
    super()
    this.setTable('ce_faqs').setName('Faqs')
    .setTimeStamps();
  ...
  renderHomeView = async (req, res) => {
    try {
      ...
      if (role == 1 || role == 2) {
        return this.renderView(res, 'modulo-generales/faqs/superadmin', {
          dataSession,
          dataSistema
        });
      }

      return this.renderView(res, 'modulo-generales/faqs', {
        dataSession,
        dataSistema
      });
