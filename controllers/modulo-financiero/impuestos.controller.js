...
class ImpuestoController extends RestBuilder {
  constructor() {
    super();
    this.setTable('impuestos').setName('Impuesto').setTimeStamps();
  }
  ...
  renderHomeView = async (req, res) => {
    try {
      ...
      if (role == 1 || role == 2) {
        return this.renderView(res, 'modulo-financiero/impuestos/superadmin', {
          dataSession,
          dataSistema
        });
      }

      return this.renderView(res, 'modulo-financiero/impuestos', {
        dataSession,
        dataSistema,
      });
