...
router.get('/items', ProductorController.index);
...
router.post('/items',
  ...
  EVResult, ProductorController.save);

router.get('/items/:id',
  check('id').isNumeric().withMessage('El id debe ser un número'),
  EVResult, ProductorController.show);
...
router.put('/items/:id',
  ...
  check('genero').optional().isString().withMessage('El campo genero debe ser un string'),
  check('empresa_id').optional().isNumeric().withMessage('El campo empresa_id debe ser un número'),
  EVResult, ProductorController.update);

router.delete('/items/:id',
  check('id').isNumeric().withMessage('El id debe ser un número'),
  EVResult, ProductorController.delete);

router.get('/datatable/:id?', ProductorController.datatable);

router.get('/', async (req, res) => {
  try {
    const { token, role, dataSession, dataSistema } = await getAllDataSession(req);
    if (role == 1 || role == 2) {
      return res.render('modulo-tv/modulo-tv/productor/superadmin', {
        dataSession,
        dataSistema
      });
    }

    return res.render('modulo-tv/modulo-tv/productor', {
      dataSession,
      dataSistema
    });
  } catch (error) {
