...
router.get('/items',  ActorController.index);
...
EVResult,  ActorController.save);

router.get('/items/:id',
check('id').isNumeric().withMessage('El id debe ser un número'), 
EVResult,  ActorController.show);
...
check('genero').optional().isString().withMessage('El campo genero debe ser un string'),
check('empresa_id').optional().isNumeric().withMessage('El campo empresa_id debe ser un número'),
EVResult,  ActorController.update);

router.delete('/items/:id', 
check('id').isNumeric().withMessage('El id debe ser un número'),
EVResult,  ActorController.delete);

router.get('/datatable/:id?', ActorController.datatable);

router.get('/', async (req, res) => {
  try {
    const { token, role, dataSession, dataSistema } = await getAllDataSession(req);
    if (role == 1 || role == 2) {
      return res.render('modulo-tv/modulo-tv/actor/superadmin', {
        dataSession,
        dataSistema
      });
    }

    return res.render('modulo-tv/modulo-tv/actor', {
      dataSession,
      dataSistema
    });
  } catch (error) {
